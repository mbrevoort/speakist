...
interface TopbarProps {
   ...
   *  admin layout passes <AdminMobileNav>. The avatar is pinned right
   *  with `ml-auto` so the layout stays correct whether `mobileNav` is
   *  passed, omitted, or hidden by its own md:hidden class above the
   *  breakpoint (justify-between alone doesn't, because hidden flex
   *  children don't claim layout space). */
...
export function Topbar({
  ...
  return (
    <header className="h-16 shrink-0 border-b border-border/70 bg-background/70 backdrop-blur flex items-center gap-3 px-4 sm:px-6">
      {mobileNav}
